...
import { Globe, RefreshCw, MapPin, Zap, Wifi, WifiOff } from 'lucide-react';
...
function generateThreat(isNew = false): ThreatOrigin {
  const severities: ThreatOrigin['severity'][] = ['critical', 'high', 'medium', 'low'];
  const pt = WORLD_POINTS[Math.floor(Math.random() * WORLD_POINTS.length)];
  return {
...
function generateThreats(): ThreatOrigin[] {
  return Array.from({ length: 6 + Math.floor(Math.random() * 5) }, () => generateThreat());
}
...
export function ThreatMapVisualization() {
  ...
  const updateVectors = useCallback((threatList: ThreatOrigin[]) => {
    setVectors(threatList.map((th, i) => ({
      id: `vec-${th.id}`,
      from: th,
      to: TARGET,
  ...
  const refresh = useCallback(() => {
    ...
    setThreats(t);
    updateVectors(t);
    setTotalAttacks(prev => prev + t.reduce((s, th) => s + th.count, 0));
  }, [updateVectors]);

  useEffect(() => { refresh(); }, [refresh]);

  // Real-time WebSocket via Supabase Realtime channel
  useEffect(() => {
    if (!isLive) {
    ...
    const channel = supabase.channel('threat-map-live', {
      config: { broadcast: { self: true } },
    });

    channel
      .on('broadcast', { event: 'new-threat' }, (payload) => {
        const newThreat = payload.payload as ThreatOrigin;
        setThreats(prev => {
          const updated = [{ ...newThreat, isNew: true }, ...prev.slice(0, 14)];
          updateVectors(updated);
          return updated;
        });
        setTotalAttacks(prev => prev + newThreat.count);
      })
      .subscribe((status) => {
    ...
    // Simulate incoming threats every 3 seconds
    const interval = setInterval(() => {
      const newThreat = generateThreat(true);
      channel.send({
        type: 'broadcast',
        event: 'new-threat',
        payload: newThreat,
  ...
  }, [isLive, updateVectors]);

  // Clear "isNew" flag after animation
  useEffect(() => {
    const timer = setTimeout(() => {
      setThreats(prev => prev.map(t => ({ ...t, isNew: false })));
    }, 2000);
              ...
              <CardTitle className="flex items-center gap-2">
                ...
                {wsConnected && (
                  <Badge variant="outline" className="text-green-500 border-green-500/30 text-[10px]">
                    <Wifi className="h-3 w-3 mr-1" />WebSocket Connected
                  </Badge>
                )}
              </CardTitle>
              <CardDescription>Geographic origins of detected threats with live attack vectors</CardDescription>
            </div>
            <div className="flex gap-2 items-center">
              <span className="text-xs text-muted-foreground">{totalAttacks.toLocaleString()} total attacks</span>
              <Button variant={isLive ? 'default' : 'outline'} size="sm" onClick={() => setIsLive(!isLive)}>
                {isLive ? <Zap className="h-4 w-4 mr-1 animate-pulse" /> : <WifiOff className="h-4 w-4 mr-1" />}
        ...
        <CardContent>
            ...
            <svg viewBox="0 0 800 400" className="w-full h-auto" style={{ minHeight: 300 }}>
                ...
                return (
                  <g key={v.id}>
                    <line x1={from.x} y1={from.y} x2={target.x} y2={target.y}
                      stroke={severityColor[v.from.severity]} strokeWidth={v.from.isNew ? 3 : 1.5} strokeOpacity={v.from.isNew ? 0.8 : 0.4}
                      strokeDasharray="6 4">
                      <animate attributeName="stroke-dashoffset" from="20" to="0" dur="1.5s" repeatCount="indefinite" />
                    </line>
                    <circle r={v.from.isNew ? 5 : 3} fill={severityColor[v.from.severity]}>
                      <animateMotion dur={v.from.isNew ? '1s' : '2s'} repeatCount="indefinite"
                        path={`M${from.x},${from.y} L${target.x},${target.y}`} />
                    </circle>
                  </g>
                );
              })}
              ...
              {threats.map((th) => {
                ...
                return (
                  <g key={th.id}>
                    {th.isNew && (
                      <circle cx={pos.x} cy={pos.y} r={20} fill="none" stroke={severityColor[th.severity]} strokeWidth={2} opacity={0.8}>
                    ...
                    <circle cx={pos.x} cy={pos.y} r={th.isNew ? 5 : 3} fill={severityColor[th.severity]} />
          ...
          <div className="mt-4 space-y-1 max-h-48 overflow-y-auto">
            {threats.sort((a, b) => {
              ...
              <div key={th.id} className={`flex items-center justify-between p-2 rounded text-xs transition-all duration-500 ${th.isNew ? 'bg-primary/10 ring-1 ring-primary/30' : 'bg-muted/30'}`}>
                <div className="flex items-center gap-2">
                  ...
                  {th.isNew && <Badge className="text-[10px] bg-primary animate-pulse">NEW</Badge>}
                </div>
                <div className="flex items-center gap-2">
