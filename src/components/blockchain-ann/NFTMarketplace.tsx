...
import { Image, ShoppingCart, Plus, TrendingUp, Heart, ExternalLink, RefreshCw } from 'lucide-react';
...
const rarityColor: Record<string, string> = {
  common: 'text-muted-foreground', uncommon: 'text-green-500', rare: 'text-blue-500',
  epic: 'text-purple-500', legendary: 'text-yellow-500',
};

export function NFTMarketplace() {
  const { toast } = useToast();
  const [nfts, setNfts] = useState<NFT[]>(SEED_NFTS);
  const [dbNfts, setDbNfts] = useState<NFT[]>([]);
  ...
  const loadNfts = async () => {
    try {
      const { data } = await supabase.from('nfts').select('*').order('created_at', { ascending: false });
      if (data && data.length > 0) {
        const mapped: NFT[] = data.map((d: any) => ({
          id: d.id, name: d.name, collection: d.collection, image: d.image,
          price: Number(d.price), currency: d.currency, owner: d.owner, creator: d.creator,
          listed: d.listed, likes: d.likes, rarity: d.rarity,
          attributes: d.attributes as { trait: string; value: string }[],
  ...
  const allNfts = [...dbNfts, ...nfts.filter(n => !dbNfts.some(d => d.name === n.name))];
  ...
  const mintNft = async () => {
    ...
    const emojis = ['🎨', '⚡', '🌀', '💎', '🔮', '🌟', '🦋', '🔷'];
    const collections = ['Quantum Defenders', 'ANN Genesis', 'Crypto Arts', 'Cyber Canvas'];
    const newNft: NFT = {
      id: crypto.randomUUID(),
      name: `QNT #${Math.floor(Math.random() * 9999)}`,
      ...
      currency: 'ETH',
      owner: `0x${crypto.randomUUID().slice(0, 6)}`,
      creator: `0x${crypto.randomUUID().slice(0, 6)}`,
      listed: true,
      likes: 0,
      rarity: rarities[Math.floor(Math.random() * rarities.length)],
      attributes: [
        { trait: 'Quantum Fidelity', value: (0.9 + Math.random() * 0.1).toFixed(3) },
        { trait: 'Layer Count', value: String(Math.floor(Math.random() * 20 + 1)) },
      ],
    ...
    if (user) {
      await supabase.from('nfts').insert({
        user_id: user.id, name: newNft.name, collection: newNft.collection,
        image: newNft.image, price: newNft.price, currency: newNft.currency,
        owner: newNft.owner, creator: newNft.creator, listed: newNft.listed,
        likes: newNft.likes, rarity: newNft.rarity, attributes: newNft.attributes as any,
        blockchain_hash: newNft.blockchain_hash,
      });
    }

    setNfts(prev => [newNft, ...prev]);
    setIsMinting(false);
    toast({ title: 'NFT Minted! 🎨', description: `${newNft.name} (${newNft.rarity}) created` });
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Image className="h-6 w-6 text-pink-500" />NFT Marketplace
              </CardTitle>
              <CardDescription>Mint, trade, and collect quantum-secured NFTs</CardDescription>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={loadNfts}><RefreshCw className="h-4 w-4" /></Button>
              <Button onClick={mintNft} disabled={isMinting}>
                <Plus className="h-4 w-4 mr-2" />{isMinting ? 'Minting...' : 'Mint NFT'}
              </Button>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-4 gap-4 mb-4">
            <div className="p-3 rounded-lg bg-muted/50 text-center">
              <div className="text-2xl font-bold text-primary">{allNfts.length}</div>
              <div className="text-xs text-muted-foreground">Total NFTs</div>
            </div>
            <div className="p-3 rounded-lg bg-muted/50 text-center">
              <div className="text-2xl font-bold text-green-500">{allNfts.filter(n => n.listed).length}</div>
              <div className="text-xs text-muted-foreground">Listed</div>
            </div>
            <div className="p-3 rounded-lg bg-muted/50 text-center">
              <div className="text-2xl font-bold text-yellow-500">{allNfts.reduce((s, n) => s + n.price, 0).toFixed(1)} ETH</div>
              <div className="text-xs text-muted-foreground">Total Volume</div>
            </div>
            <div className="p-3 rounded-lg bg-muted/50 text-center">
              <div className="text-2xl font-bold text-purple-500">{new Set(allNfts.map(n => n.collection)).size}</div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex gap-1">
          {['all', 'legendary', 'epic', 'rare', 'uncommon', 'common'].map(r => (
            <Button key={r} variant={filter === r ? 'default' : 'outline'} size="sm" className="text-xs" onClick={() => setFilter(r)}>{r}</Button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(nft => (
          <Card key={nft.id} className="overflow-hidden hover:border-primary/50 transition-all">
            <div className="h-32 bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center text-5xl">{nft.image}</div>
            <CardContent className="p-4">
              ...
              {nft.blockchain_hash && (
                <div className="text-[9px] font-mono text-muted-foreground mb-2 truncate">🔗 {nft.blockchain_hash.slice(0, 24)}...</div>
              )}
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="flex items-center gap-1 text-xs text-muted-foreground"><Heart className="h-3 w-3" />{nft.likes}</span>
                  {nft.listed && (
